...
* **SMB Vulnerability Mitigation:** Disables **File and Printer Sharing** rules and disables the `LanmanServer` (Server) service to **close ports 139 and 445** against exploit tools like **Metasploits**. This service allows the attacker to perform attacks on the victims computr through Local Wifi Network.
* **Remote Access Securing:** Disables **WinRM** (Windows Remote Management) and the **RemoteRegistry** service to prvent lateral movement and unauthorized registry manipulation on the computer.
...
2. Right-click `secure.bat` and select **Run as administraitor**.
3. Verify output:
